...
class LicensePlateApp(object): # QMainWindow 상속을 제거하여 꼬임 방지
    ...
    def update_pipeline(self):
        ...
        # --------------------------------------------------
        # 5. [시연 및 결과 출력] UI 화면 실시간 매핑 및 베스트 선별 저장
        # --------------------------------------------------
        ...
        if cropped_plate is not None:
            try:
                # 원래 기욱님 모델이 잘라준 원본 크기 확인
                orig_h, orig_w = cropped_plate.shape[:2]
                
                # 1) 번호판 숫자가 잘리지 않도록 테두리 여백(Padding) 추가 (사방 15픽셀 흰색 마진)
                padding_size = 15
                padded_plate = cv2.copyMakeBorder(
                    cropped_plate, 
                    top=padding_size, bottom=padding_size, 
                    left=padding_size, right=padding_size, 
                    borderType=cv2.BORDER_CONSTANT, 
                    value=[255, 255, 255]
                )
                
                # 2) [🖥️ GUI 실시간 표시] 여백이 확보된 이미지를 오른쪽 하단에 출력
                self.display_image(padded_plate, self.screen_crop)
                
                # 3) [사이클 버퍼링] 추적 상태가 아니었다면 새 사이클 초기화
                if not self.is_tracking:
                    self.is_tracking = True
                    self.cycle_images = [] 
                    print("\n==================================================")
                    print("🎯 [TRACKING START] 새로운 차량 포착! 이미지 수집을 시작합니다.")
                
                # 🚀 [필터링 주입]: 원본의 가로세로 비율(w/h)을 계산하여 잘린 파편은 버퍼에서 아예 제외합니다.
                if orig_h > 0:
                    aspect_ratio = orig_w / orig_h
                    # 대한민국 자동차 번호판의 정상 비율 범주 필터 (너무 길거나 너무 높게 잘린 파편 필터링)
                    if 2.8 <= aspect_ratio <= 5.2:
                        self.cycle_images.append(padded_plate)
                
            except Exception as e:
                print(f"🚨 [프로세스 오류] 실시간 이미지 여백 처리 중 예외 발생: {e}")
                
        else:
            # 4) [🏁 차량이 화면을 탈출하여 사이클이 끝난 시점]
            if self.is_tracking:
                self.is_tracking = False 
                print("🏁 [TRACKING END] 차량이 화면을 벗어났습니다. 수집된 정형 프레임 분석을 시작합니다.")
                
                if self.cycle_images:
                    try:
                        # 정상 비율의 버퍼 프레임 중 가장 면적이 넓은(정면에 가까운) 베스트 컷 선별
                        ...
                        filename = f"best_plate_{timestamp}_{millis:03d}.png"
                        filepath = os.path.join(save_dir, filename)
                        
                        # OpenCV 이미지 최종 기록
                        success = cv2.imwrite(filepath, best_plate)
                        if success:
                            print(f"💾 [BEST SAVE SUCCESS] 파편 제외 {len(self.cycle_images)}개 후보 중 최적의 베스트 컷 저장 완료!")
                            print(f"📌 저장 경로: {filepath}")
                        else:
                            print(f"❌ [SAVE FAILED] 파일 작성 실패: {filepath}")
                            
                    except Exception as save_err:
                        print(f"❌ [선별 오류] 베스트 컷 연산 및 저장 중 실패: {save_err}")
                        
                    # 버퍼 비우기
                    self.cycle_images = []
                else:
                    print("⚠️ [SYSTEM WARNING] 이번 사이클에서는 짤린 파편 프레임만 검출되어 저장을 건너뛰었습니다.")
                print("================================================  \n")
